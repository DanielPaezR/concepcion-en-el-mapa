...
export default function MisReservas() {
    ...
    const getEstadoInfo = (estado) => {
        switch(estado) {
            case 'pendiente': return { icon: <ClockIcon className="w-5 h-5 text-yellow-500" />, text: 'Pendiente', color: 'bg-yellow-100 text-yellow-800' };
            case 'confirmada': return { icon: <CheckCircleIcon className="w-5 h-5 text-green-500" />, text: 'Confirmada', color: 'bg-green-100 text-green-800' };
            case 'completada': return { icon: <CheckCircleIcon className="w-5 h-5 text-blue-500" />, text: 'Completada', color: 'bg-blue-100 text-blue-800' };
            case 'cancelada': return { icon: <XCircleIcon className="w-5 h-5 text-red-500" />, text: 'Cancelada', color: 'bg-red-100 text-red-800' };
            default: return { icon: <AlertCircle className="w-5 h-5 text-gray-500" />, text: estado, color: 'bg-gray-100 text-gray-800' };
        }
    };

    const reservasFiltradas = reservas.filter(r => {
        if (filtro !== 'todas' && r.estado !== filtro) return false;
        if (busqueda && !r.lugar_nombre?.toLowerCase().includes(busqueda.toLowerCase())) return false;
        return true;
    });
                            ...
                            <div key={reserva.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                                <div className="flex justify-between items-start">
                                    <div className="flex-1">
                                        ...
                                        {reserva.guia_nombre && (
                                            <p className="text-xs text-green-600 mt-2">🧙‍♂️ Guía: {reserva.guia_nombre}</p>
                                        )}
                                    </div>
